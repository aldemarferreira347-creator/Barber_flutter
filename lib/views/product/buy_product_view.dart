  ...
  Future<void> _buy() async {
    ...
    try {
      ...
      final id = await repo.createPurchase(
        ...
        items: [PurchaseItemInput(productId: widget.product.id, quantity: _quantity)],
      );
  ...
  Widget _buildStatus(String purchaseId) {
    ...
    return StreamBuilder<Purchase?>(
      ...
      builder: (context, snapshot) {
        ...
        if (purchase == null || purchase.status == PurchaseStatus.pendingPayment) {
          return const Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                ...
                SizedBox(height: 16),
                Text('Confirma el pago en tu app Nequi...', textAlign: TextAlign.center),
              ],
            ),
          );
        }
        if (purchase.status == PurchaseStatus.paymentFailed) {
          return const Center(
            child: Text('El pago no pudo procesarse. Intenta de nuevo.', style: TextStyle(color: AppColors.error)),
          );
        }
        return Center(
          child: Column(
            ...
            children: [
              ...
              const Text('¡Compra confirmada!', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
